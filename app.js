...
Order.belongsToMany(Product,{through:OrderItem}); //note: the Product model class có modelName là 'SANPHAM', khi được Sequelize pluralizes hay người lập trình "eager loading" sẽ thành SANPHAMs (chứ dùng Products là báo lỗi). VD trong createSANPHAM() hay getSANPHAMs() , và req.user.getOrders({include:['SANPHAMs']})
//a through model which is to join source and target in n:m associations
