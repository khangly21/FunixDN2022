...
//lấy hàm session() để tạo session 
...
//import User model / class (constructor) nên viết hoa U
...
//INITIALIZE SESSION with middleware 
//I am looking for a cookie to come with a request, dựa vô đó tui sẽ tìm session tương ứng và load data

//Câu hỏi phổ biến là nếu không có session middleware thì req tới trang bất kỳ có req.session không? Không
//Bất cứ req nào gặp session() đều là ĐẦU VÀO và nhận thuộc tính req.session và req.sessionStore
app.use(session({
      
      //app.user() nhận request handler, và middleware session() cũng là  1 request handler
      //https://www.geeksforgeeks.org/express-js-app-use-function/ (callback inside app.use)
...
//Kết quả: sau khi npm start thì session() chạy và tạo 1 collection tên "sessions" trên MongoDB
//Nhấn mạnh là npm start thì chạy ngay session() không có đợi req nào hết, vì không giống trường hợp sau phải chờ có req thì mới chạy callback: https://www.geeksforgeeks.org/express-js-app-use-function/
/*
...
app.use(function (req, res, next) {
    ...
    next();
}); //dựa trên công thức app.use(path,callback) thì callback phải đợi req tới thì mới gọi callback
...
//công thức là app.use(path,middleware callback)
