...
//INITIALIZE SESSION with middleware ( this middleware automatically sets a cookie for you, and it automatically reads the cookie value for you too, so it does all the cookie parsing and setting for you)
//register the "miniapp" session middleware to app for any incoming REQUEST. we pass 'session' and we EXECUTE this as a function which receive an js object where we configure the session setup as parameter 
...
//Đây là script tự động quản lý session
   /// the session is already something which will be managed for us automatically and for the incoming requests, we register the middleware which  will then basically look for a session cookie (được gửi chung với request), if it finds one it will look for a fitting session in the database and load the data from there.
...
//Kết quả thử nghiệm 1:
   /// xóa collection "sessions" trên MongoDB, npm start thì trên MongoDB có sessions collection
   /// session() là 1 middleware nhận req object , rồi nhìn vào session cookie chứa sessionId rồi tìm session tương ứng trên MongoDB, tìm ra thì biết được session chứa sensitive data gì, và middleware tiếp theo là "general middleware" có thể sử dụng session data 
//Kết quả thử nghiệm 2:
   /// xóa collection sessions và client cookies
   /// thêm thuộc tính cho req.session ngay bên trong session() //req.session:"session xin chào", thất bại do báo lỗi token '.'
...
app.use((req,res,next)=>{
    //code bên trong tương tự (nhưng có trùng lặp??) controllers >> auth >> postLogin
    User.findById('62c4c9cce3789b7b623b3ba8')  
    .then(user => {
        // Here we don't want to store anything in the session như req.session.isLoggedIn=true; hay req.session.user = user; Chỉ có khi postLogin thì mới gắn biến cho session

        // this means that now we just want to use that session data to load our real user, to create our mongoose user model
        // xem sơ đồ sau https://afteracademy.com/blog/expressjs-middleware-in-javascript


        next(); //we'll call next so that the incoming requests can continue with the next middleware. next() là chuyển tiếp xuống đây. Nếu next(err) thì express mặc định hiểu là chuyển lỗi từ trên xuống
    })
