...
Cart.belongsToMany(Product, {through:CartItem}); //"through" keep telling sequelize where these connection should be stored and that is my CartItem model
...
.then(
    ...
    user=>{
        //console.log(user); 
        //server is listening
        app.listen(3001); //Sau npm start lần 1 thì bảng rỗng đã được thêm dòng đầu tiên, check code if(!user) bằng cách npm start lần 2 , refresh bảng users không thấy dòng mới vì đã tồn tại model instance ở ID=1 nên không create nữa
