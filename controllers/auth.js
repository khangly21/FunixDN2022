exports.getLogin = (req, res, next) => {
      ...
      //trong trường hợp tác giả thì còn có extension nữa thì tổng cộng có 2 cookies:  
         /// io=--YkAezJV_KJJJJ ; loggedIn=true 
         /// thì sẽ trích ra bằng cách lấy phần tử index 1 của mảng console.log(req.get('Cookie').split(';')[1]);  nhận được "loggedIn=true "
         // split() splits a string into an array of substrings, and returns the array req.get('Cookie').split(';')
         // trim that to remove any excess white spaces and split again on "=" sign
         // console.log(  req.get('Cookie').split(';')[1].trim().split('=')[1]  );  sẽ nhận được "true" value

      //cổng Login là nơi đầu tiên nhận cookie do trình duyệt gửi lên
      // nếu req.get('Cookie') mà undefined thì khi vào menu Login, thì TypeError: Cannot read properties of undefined (reading 'split')
      let isLoggedIn;
      if(req.get('Cookie')){
         isLoggedIn=req
         .get('Cookie')
      ...
      console.log(isLoggedIn); //Khi trở lại trang login thì hành động getLogin sẽ log ra "true"
