    ...
    User.findOne({email:email})
        .then(userDocumentModel=>{
            ...
            if(userDocumentModel){
                ...
                return res.redirect('/signup'); //Anh cần đăng ký mới. Nhưng problem là chạy dần xuống dưới sẽ gặp res.redirect('/login') và bị báo lỗi "Cannot set headers after they are sent to the client"
            ...
            //đã successfully passed this check, we know that no user with that e-mail exists yet and therefore we can create a new one

            //I store a hashed password, first param is string to be hashed, second param is the salt value which is how many rounds for hashing,  a value of 12 is accepted as highly secure.
            //this is an asynchronous task (task cần nhiều thời gian không thực hiện bởi NodeJS mà bởi engineV8 của Chrome HOẶC Edge Chrome) and therefore this actually gives us back a promise.
            
            return bcrypt.hash(password,12);  //trả về "future" value là futureHashedPassword
