...
exports.getIndex = (req, res) => {
  console.log("req object của  getIndex có gì nào?? ", req)
  if(req.session){
      console.log("Bỏ app.use và session() đi, req.session có là mặc định sẵn ?")
      //console.log("getindex vào menu Shop: \n",req.session); 
  }else{
      console.log("req.session có là không có sẵn") //không có app.use(session())thì dòng này hiện lên, vì req sẽ không có req.session và req.sessionStore
  } 
...
exports.getCart = (req, res, next) => {
  ...
  req.user
    .populate('cart.items.productId')   //Error: 500 , req.user.populate is not a function
    //không trả về Promise
