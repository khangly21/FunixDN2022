...
exports.postOrder=(req,res,next)=>{
    let fetchedCart; //ban đầu undefined variable, tác giả nói "empty"
    ...
    req.user
       .getCart()
       .then(cart=>{
          //now I can access to the cart.I store the cart in fetchedCart 
          fetchedCart=cart;
       ...
       .then(products=>{
          ...
          return req.user.createOrder()
                         ...
                         .then(any_result=>{
                           //the cart should essentially drop all its cart items. Now I can use my fetchedCart here to call another method
                           //we dropped all the items in the cart by setting them to null,
                           return fetchedCart.setSANPHAMs(null);
...
exports.getOrders = (req, res, next) => {
   //a magic method added by sequelize, thanks to our associations
  
   req.user.getOrders({include:['SANPHAMs']}) //fetching all order of this user, each order includes "products" property => Error: Association with alias "Products" does not exist on order
            //Kết quả: Each order will now have a SANPHAMs array, mỗi SANPHAM trong mảng sẽ qua hàm forEach để render ở view orders.ejs
            .then(orders=>{ //we got orders with more data in them, chính là SANPHAMs
               
               console.log("my Order: \n",orders); //xem cấu trúc của orders để có thể render ra view, thấy orderitem không có thuộc tính orderItem là bảng trung gian (trong khi cartitem sẽ mang thuộc tính cartItem)
               res.render('shop/orders', {
                  path: '/orders',
                  pageTitle: 'Your Orders',
                  //I add a new property to carry the returned orders 
                  orders: orders //mỗi order model instance sẽ có {id, createdAt,updatedAt, userId}
               })
            })
            .catch(err=>console.log(err));
   ...
   req.user.getOrders()
      .then(orders=>{
         //One-To-Many, không có hàm orders.getSANPHAMs()
          return orders.getSANPHAMs() //orders chỉ chứa các model instance order cùng với thuộc tính thêm vào là userId:1 
      })
      .then(products=>{
         console.log("orderitem từ order là: \n",products)
      })
      .catch(err=>console.log(err));
   */
}
