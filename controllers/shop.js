...
exports.getProducts = (req, res, next) => {
  ...
  //TRƯỜNG HỢP 1: Để isLoggedIn undefined sau đó gán có điều kiện
     ///  // thì console log trong server ra : "undefined" || giá trị
     /// let isLoggedIn;
     ///     if(req.get('Cookie')){
     ///        isLoggedIn=req
     ///        .get('Cookie')
     ///        .split('=')[1] //trường hợp chỉ có 1 cookie
     ///     }
  //TRƯỜNG HỢP 2: cho undefined và không gán. TH này sẽ log ra "undefined"
      let isLoggedIn;

  //TRƯỜNG HỢP 3: không có biến nào tên isLoggedIn  . Trường hợp này server throws error 500 (Internal server error). Người dùng thấy Internal Server Error, còn dev thấy " Error: isLoggedIn is not defined "

  console.log("controllers shop  getProducts: \n",isLoggedIn); //s%3AF3ND6dfyZRQUdbttbH0fcrGCmdEzGkBi.TY2XRDBKpklAM1DUKMP2uZqbLDgxwxbewVML01Xla7k
  ...
  .then(products=>{
    ...
    res.render('shop/product-list', {
      ...
      path: '/products',
      isAuthenticated:req.session.isLoggedIn   //bình thường nếu isAuthenticated:isLoggedIn thì if(isAuthenticated) thì sẽ rẽ nhánh true khi đó chưa log in nhưng lại xuất hiện 2 menu Admin Products và Add Products . Nên phải cho false
    });
  })
  .catch(err => {
    console.log(err);
...
exports.getIndex = (req, res, next) => {
  ...
  Product.find()
    .then(products => {
      ...
      //Nếu trên trình duyệt Dev Tool >> Application có 1 cookie thì phải xóa, vì nếu không isAuthenticated:true thì mới trang chủ đã hiện Admin Products và Add Products
      // let isLoggedIn;
      // if(req.get('Cookie')){
      //    isLoggedIn=req
      //    .get('Cookie')
