...
exports.postAddProduct = (req, res, next) => {
  ...
  */
  ...
  Product.create({
    //hover hàm create thấy nó là một `async function` tạo ra một Promise object 
       /// do đó quá trình create sẽ mất một thời gian dài mới xong ,nên có thể bị điều khiển của chương trình bỏ qua, đi tới những code không tốn thời gian chạy. 
       /// đối với hàm async thì sẽ dùng Promise hay await operator for async function to wait for a Promise
       /// const product = await Product.findByPk(123);  //trong đó findByPk(Id) là một async function which return a Promise
    //Product là Model which is ES6 class nên Product là class 
       /// Khi hover lên Product thấy nó là biến const có kiểu dữ liệu Sequelize.ModelCtor nay đã bị deprecated mà dùng ModelStatic 
       /// nên rất có thể Sequelize.ModelCtor chính là một class của thư viện Sequelize (mentor Ngô Tuấn Anh nói) hoặc là một model construtor  
    //create() hay build() sẽ to build a new model instance and calls save on it, lý do là Product không có hàm save()
  ...
  }) //const jane = await User.create({ name: "Jane" }); tạo ra jane là Model instance
  .then(result=>{
    //in case it does not go wrong.
    //console.log(result);
    console.log('Created Product'); //chờ tới khi Model instance được tạo hoàn tất thì dòng này mới hiện ra
