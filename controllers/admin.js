//import Product model, vì User loại admin chỉ thực hiện actions trên Product
//User có mặt trong tất cả các hàm xử lý trong controllers , vì nó nằm trong req.user
...
//Tóm lại nhiệm vụ của Controller là gì?
   ...
   /*
       ...
       Then we create the CONTROLLER for creating and retrieving/findAll/findByPk từ CSDL Entities. Nói chung là take actions on Entity/Model class
    ...
    //execute sql khi GET tới trang này bằng nhấn nút edit của SP id=2 là: SELECT * FROM SANPHAMs AS SANPHAM WHERE SANPHAM.userId=1 AND SANPHAM.id=2
      ...
      .then(products=>{
        ...
        //keep in mind here we get back an array even if it only holds one element (bởi 1 người có thể create nhiều sanpham), do đó we get "products". the one we are interested in will always be the first element
           ...
           const product=products[0]; // đối tượng product có thể không tồn tại trong trường hợp products là mảng rỗng
           //không cần nhắc tới dataValues , we have to store that separately in a new constant
           if(!product){
