...
exports.postDeleteProduct=(req,res,next)=>{
    ...
    Product.findByPk(prodId)
    .then(
        //we know we got a product 
        //on that product, we call destroy the found product
        product=>{
            return product.destroy(); //return Promise which calls the following then()
        }
     ) 
    .then(
       //be called once the destruction succeeded
        result=>{
             console.log('DESTROYED PRODUCT');
             //để res.redirect chỗ này thì tránh ASYNC thực hiện res.redirect trước rồi đợi hàm trong then sẵn sàng mới chạy. //We should redirect here to make sure we only redirect once the deletion succeeded
        }
    )
    .catch(err=>console.log(err))
