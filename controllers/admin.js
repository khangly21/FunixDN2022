...
exports.postAddProduct = (req, res, next) => {
  ...
  const product = new Sanpham({
    //the part on the left side of the colon refers to the keys you defined in your schema, trong object thì thứ tự nào cũng được
      ...
      imageUrl:imageUrl,
      //userId:req.user._id  là cách thông thường nhất
      //Advanced: conveniently in mongoose, you can even just store the entire user object, so this is really the entire user object not just the ID and mongoose will just pick the ID from that object
      userId:req.user
  }); 
...
exports.getEditProduct = (req, res, next) => {
  ...
  */
  ...
  //hàm findById là built-in Query của Mongoose
...
exports.getProducts = (req, res, next) => {
  ...
  Sanpham.find()
    //select() specifies which doc FIELDS to include or exclude (also known as the query 'projection')
    //you could then INCLUDE title and price like this and you could even EXCLUDE something like the ID by adding a minus in front of it
    .select('title price -_id') //SPECIAL note: the ID will by the way always be retrieved unless you explicitly exclude it.
    //nếu .select('title price - _id') là báo lỗi MongoServerError: FieldPath cannot be constructed with empty string
    .populate('userId','email') 
    //Mongoose utility method to specify paths which should be populated (=fully enriched information) with other documents
    /*
       Populate allows you to tell mongoose to populate a certain field with ALL the detail information 
       and NOT just the ID-related information
    */
    // first of all describe the path which you want to populate
    //with populate() --> field selection for the population query
    //the same can be done on populate by passing a second argument, this would be a string where you say ok I want to get the name
