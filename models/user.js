...
const userSchema= new Schema({
    ...
    cart:{
        ...
        items:[
            //use references with user:cartitem
            {
                //Cách đặt tên: productId thì trong ngữ cảnh reference, còn khi populate(cart.items.prodId) thì reference sẽ chuyển thành embed toàn đối tượng lúc này có thể đổi tên thành product
...
//thêm hàm addToCart vào nhóm các hàm đã định nghĩa của userSchema (object userSchema.methods)
/*
//controllers >> shop.js  chỉ cần gọi service addToCart, còn logic là gì thì Model lo hết
...
userSchema.methods.addToCart=function(product){//Why product parameter? we just need to ASSUME because we are the one writing the code in the end. (Trong javascript khi lần đầu cho 1 parameter vào nó sẽ undefined, nhiệm vụ người viết là giúp JS nhận ra project là một Product instance trong thân hàm addToCart)
    ...
    //code should still work for getting the product index
    //using array.findIndex() instead of array.find() for 03 reasons
    //Reason 1: mảng trong Javascript cũng có hàm find() nhưng trả về undefined nếu không tìm ra, khi đó không định vị được product chỗ nào nên không tiếp cận được quantity trong cart! 
    ...
    //In Mongoose, the Model.find() function is the primary tool for querying the database, không trả về cursor mà trả về mảng products
       /// trong controllers>>admin thì khi controller gọi dịch vụ Sanpham.find().then(products=>)
    //Reason 2: vì array.findIndex cũng trả về index, nên nếu cartProductIndex = -1 thì không có SP trong cart
    //Reason 3: nếu có sp trong cart thì access lấy từ this.cart.items[cartProductIndex]
