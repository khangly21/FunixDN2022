...
//Why you need schema? 
    ///to create the model / class for data  ( nơi xác định và validate dữ liệu trông như thế nào ) for our API / controller which is middleware function between and process req .
    /// nói về validation, VD userId không được gán gì hết thì ValidationError: Sanpham validation failed : userId: Path `userId` is required
const productSchema = new Schema({
    //mặc định là MongoDB luôn tạo tự động _id , muốn không tạo tự động mà làm manual thì không phải required:false, mà là Có thể định nghĩa _id cho một Document khi thêm định nghĩa sau lúc tạo Schema: { _id: false } 
    title: {
    ...
    userId:{
        ...
        ref:'User', //chú ý: không có import User Model. The 'ref' option is what tells Mongoose which model to use during population
        required:true
    }
});
