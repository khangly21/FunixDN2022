...
const getDb=require('../util/database').getDb; //allows us to have MongoDB access (bằng cách yêu cầu _db kết nối CSDL tên gì??)

//references trong Product class có các lần gọi hàm static, trong khi không có tính các lần instances gọi nonstatic
class Product{
    ...
    //Product constructor có 2 references, nghĩa là được gọi 2 lần  new product instance
    constructor(title,price,description,imageUrl,id,userId){  //admin user thường không gửi id bằng input hidden thông qua postAddProduct như postEditProduct, vì trong postAddProduct sẽ có _id object tự động tạo bởi mongodb.ObjectId class
        ...
        //tài liệu mongodb nói có thể không cần "new" operator , gọi trực tiếp mongodb.ObjectId(id)
        this._id=id ? new mongodb.ObjectId(id) : null  //nếu postEditProduct thì có string id nên this._id lưu ObjectId id, còn postAddProduct thì không gửi STRING id qua input hidden qua đây nên this_id lưu giá trị null
    ...
    save(){
        //hàm nonstatic này có 2 references nghĩa là được instance gọi vd const product= new Product(,,,) sau đó product.save()
    ...
    static fetchAll(){
        ...
        return db
            .collection('products')
            .find()
            .toArray() //array thực sự là chứa các access tới dữ liệu 
            .then(products=>{
