...
class Main extends React.Component {
    ...
    render(){
        ...
        console.log(this.state.staffs.filter((staff) => staff.id === this.state.Ma_so_nhan_vien_duoc_chon)[0]);  
            ...
            return(
                //ReactRouterDOM.HashRouter đã bao quanh App, App chứa Main
                //tiếp theo thiết lập ReactRouterDOM.Link to và 
                //thao khảo cấu trúc Main của Lab06_3_react_redux_form bao gồm Header, các Route_path bao lấy Components và path, Footer
                    /// trong bài này thì các "Link to"/"NavLink href #" / "a href #" sẽ tập trung ở headerComponent.js và footerComponent.js ở chỗ thanh menu
                //<ReactRouterDOM.Route exact path="/nhan_vien" component={<WebBody_of_Employees departments={this.state.departments} staffs={this.state.staffs}  data={this.state.Chuoi_JSX_tat_ca_Nhan_vien} />}/> => Warning: React.createElement: type is invalid -- expected a string (for built-in components) or a class/function (for composite components) but got: <WebBody_of_Employees />. Did you accidentally export a JSX literal instead of a component?
                   /// composable --> composite component (nghĩa là component chứa các component con)
                //https://reactgo.com/react-router-pass-props/
                //Nếu tới "/" không hợp Route nào thì Body rỗng, Header và Footer dính nhau
                //<ReactRouterDOM.Route exact path="/" component={Main}/> hay <ReactRouterDOM.Route exact path="/" component={this}/> sẽ treo trang web
                <div>
                    ...
                    <ReactRouterDOM.Route exact path="/" component={None}/>
                    <ReactRouterDOM.Route exact path="/nhan_vien" children={<WebBody_of_Employees departments={this.state.departments} staffs={this.state.staffs}  data={this.state.Chuoi_JSX_tat_ca_Nhan_vien} />}/>
                    <ReactRouterDOM.Route exact path="/phong_ban" children={<WebBody_of_Departments data={this.state.Chuoi_JSX_tat_ca_Phong_ban}/>}/>
                    <ReactRouterDOM.Route exact path="/bang_luong" children={<WebBody_of_Salaries data={this.state.Chuoi_JSX_tat_ca_Bang_luong}/>}/>
