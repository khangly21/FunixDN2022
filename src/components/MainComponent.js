...
const mapStateToProps = (state) => { //tham số state tượng trưng cho state từ Redux store, xem reducer.js trả về đối tượng state chứa 4 anh nào: dishes, comments, promotions, leaders
    ...
    return {
        ...
        dishes: state.dishes,   //state.dishes is mapped to dishes. Là đúng trong Lab08_2_Muppala, nhưng sai ở Lab09_1 vì shape của state đã thay đổi, không còn cố định DISHES nữa mà có tới 3 thuộc tính
...
const mapDispatchToProps=(dispatch)=>({//Why? mapDispatchToProps sử dụng store.dispatch() sẽ giúp Dispatches từ Store được chuyển hóa thành props cho React sử dụng. Keyword "dispatches" được tìm thấy trong ActionCreator.js
    ...
    //fetchDishes nhập từ ActionCreator chỉ là thuộc tính của đối tượng, chứ không phải gọi hàm(),  nếu được invoked sẽ returns dispatch(fetchDishes())
    fetchDishes:()=>{dispatch(fetchDishes())} //fetchDishes() creates a Thunk trong ActionCreator, và Thunk này được Store truyền cho MainComponent thông qua mapDispatchToProps
                                              // mapDispatch thực ra là truyền 2 dispatches dispatch(dishesLoading(true)) và dispatch(addDishes(DISHES)) bên trong thunk fetchDishes cho MainComponent
                                              //Vậy là từ lúc này nhận được Thunk, thì MainComponent có thể kích hoạt các ActionCreator bên trong thunk fetchDishes(), hàm ActionCreator sẽ tạo ra Action object từ đó tùy action mà quyết định bổ sung state cho Dishes reducer
})

//where does MainComponent fetch the dishes?
...
class Main extends React.Component {
    ...
    componentDidMount(){ //Why componentDidMount?
        //let ham=this.props.fetchDishes();
        this.props.fetchDishes;  //không làm theo thì không fetch dữ liệu được, chỗ này làm theo Muppala cứ bị lỗi Uncaught Error: Invariant Violation: Actions must be plain objects. Use custom middleware for async actions
        
        //I will call this function fetchDishes to fetch any data that I require, right after MainComponent is mounted into view
        //At the point after MainComponent is mounted, the  fetchDishes will be called and this will result in a call to fetch dishes and load it into my redux's store 
    }

    
    render(){

        //Các props sau đã available sau khi connect Main to mapStateToProps và mapDispatchToProps
        ...
        console.log(this.props.fetchDishes); //ok
        ...
        const HomePage3 = () => {
            ...
            return(
                ...
                //trong dishes.js thì this.props.dishes là 1 đối tượng có 3 thuộc tính, nên phải map state có đủ 3 thuộc tính vào props
                ...
                <HomePage 
                    ...
                    //The reason why I marking this separately as dish,dishesLoading,dishesErrorMessage because, the same thing will aply for promotion and leader later on
        ...
        const DishWithId = ({match}) => { 
            ...
            return(
                <DishDetails 
                    ...
                    //addComment= {(dishId,rating,author,comment)=>store.dispatch(addComment(dishId,rating,author,comment))} //không cần dispatch làm tham số ////addComment={this.props.addComment}
                />
            );
        };
                ...
                <div>
                            ...
                            <Header/>
                                ...
                                <div>
                                        ...
                                        <ReactRouterDOM.Route path='/' component={HomePage3} /> 
                                        ...
                                        <ReactRouterDOM.Route path='/menu/:dishId' component={DishWithId} />
...
const ConnectedMainComponent =ReactRouterDOM.withRouter(ReactRedux.connect(mapStateToProps,mapDispatchToProps)(Main)); //nhờ connect với 
