...
const  DishDetails = (props) => { //nhận dữ liệu props.dish từ Main là 1 đối tượng dish, có dùng map() để tạo Chuoi_JSX cho từng dish ??
    ...
    //dùng conditional rendering để làm việc với Thunk
    //nhận props từ MainComponent
    if(props.isLoading){ //true có nghĩa là chưa có dish nào , tương đương selectedDish != null
        //we often use row to positioning things on the screen using Bootstrap Grid , then we see the loading spinner on the screen
        return(
    ...
    //Finally, my dish is ready when isLoading:false  and ErrMessage:null.
    //nOW i am able to display the dish here
    else if(props.dish != null) //Mappala: Problem is when I wanna display dish={this.props.dishes.dishes} with Thunk, I realise that selectedDish maybe null. so in that case what am I supposed to do?
        return(
